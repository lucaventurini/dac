...
object L3ExampleMushroom {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("Bagging_v0.2.0").setMaster("local[*]")
    ...
    /* On all the dataset: */
    val l3 = new L3(numClasses = 3, minSupport = 0.01) //they start from 1, minsup=3000:0.369
    val model=l3.train(transactions)
    println(model)

    var writer = new PrintWriter(new File("/home/lucav/data/L3.out"))

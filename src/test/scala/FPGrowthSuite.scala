...
class FPGrowthSuite extends FunSuite {
  ...
  test("FP-Growth using String type") {
    val transactions = Seq(
      ...
      "x z y r q t p")
      .map(_.split(" "))
    //val transactions = sc.parallelize(transactions, 2).cache()
    ...
    val model6 = fpg
      ...
      .setNumPartitions(1)
      .run(transactions)
    assert(model6.freqItemsets.size === 0)

    val model3 = fpg
      .setMinSupport(0.5)
      .setNumPartitions(2)
      .run(transactions)
    val freqItemsets3 = model3.freqItemsets.map { itemset =>
    ...
    val model2 = fpg
      ...
      .setNumPartitions(4)
      .run(transactions)
    assert(model2.freqItemsets.size === 54)

    val model1 = fpg
      .setMinSupport(0.1)
      .setNumPartitions(8)
      .run(transactions)
    assert(model1.freqItemsets.size === 625)
  }
  ...
  test("FP-Growth using Int type") {
    ...
    val model3 = fpg
      ...
      .setNumPartitions(2)
      .run(transactions)
    assert(model3.freqItemsets.head.items.getClass === Array(1).getClass,
    ...
    val model2 = fpg
      ...
      .setNumPartitions(4)
      .run(transactions)
    assert(model2.freqItemsets.size === 15)

    val model1 = fpg
      .setMinSupport(0.1)
      .setNumPartitions(8)
      .run(transactions)
    assert(model1.freqItemsets.size === 65)
  }
}
